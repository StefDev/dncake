...
class Event extends AppModel {
  ...
  public function afterFind($results, $primary = false) {
    ...
    $results[0]["Event"]["descr"] = $this->formatBodyText($results[0]["Event"]["descr"]);
    
    if ($results[0]["Event"]["quotes"] && !preg_match("/^&bdquo;/", $results[0]["Event"]["descr"])) {
      $results[0]["Event"]["descr"] = sprintf("&bdquo;%s", $results[0]["Event"]["descr"]);
    }
    
    if ($results[0]["Event"]["quotes"] && !preg_match("/&rdquo;$/", $results[0]["Event"]["descr"])) {
      $results[0]["Event"]["descr"] = sprintf("%s&rdquo;", $results[0]["Event"]["descr"]);
    }
